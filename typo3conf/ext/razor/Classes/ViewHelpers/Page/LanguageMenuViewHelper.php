<?php
namespace RZ\Razor\ViewHelpers\Page;
...
class LanguageMenuViewHelper extends AbstractTagBasedViewHelper
{
    ...
    public function initializeArguments()
    {
        ...
        $this->registerArgument('defaultIsoCode', 'string', 'ISO code', false, 'en');
    ...
    protected function parseLanguageMenu()
    {
        ...
        $tempArray[0] = array(
            ...
            'uid' => 0,
            'iso' => $this->arguments['defaultIsoCode'],
        );
        ...
        foreach ($sysLanguage as $value) {
            $iso = $GLOBALS['TYPO3_DB']->exec_SELECTgetSingleRow('lg_iso_2', 'static_languages', 'uid=' . $value['static_lang_isocode'], '', '', '');

            $tempArray[$value['uid']] = array(
                'label' => $value['title'],
                'flag' => $value['flag'],
                'uid' => $value['uid'],
                'iso' => strtolower($iso['lg_iso_2']),
            );
        }
